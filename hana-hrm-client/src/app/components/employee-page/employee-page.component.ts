...
import {
  EmployeeDetailDto,
  EmployeeDocumentDto,
  EmployeeEducationInfoDto,
  EmployeeFamilyInfoDto,
  EmployeeProfessionalCertificationDto,
...
export class EmployeePageComponent implements OnInit {
  ...
  private patchForm(detail: EmployeeDetailDto): void {
    ...
    this.setFormArray(
      'employeeFamilyInfos',
      detail.employeeFamilyInfos ?? [],
      this.createFamilyInfoGroup.bind(this),
    );
    this.setFormArray(
      'employeeEducationInfos',
      detail.employeeEducationInfos ?? [],
      this.createEducationInfoGroup.bind(this),
    );
    this.setFormArray(
      'employeeDocuments',
      detail.employeeDocuments ?? [],
      this.createDocumentGroup.bind(this),
    );
    this.setFormArray(
      'employeeProfessionalCertifications',
      detail.employeeProfessionalCertifications ?? [],
      this.createProfessionalCertificationGroup.bind(this),
    );
  ...
  private createDocumentGroup(value?: EmployeeDocumentDto): FormGroup {
    return this.fb.group({
      ...
      uploadDate: [
        this.formatDate(value?.uploadDate) ?? new Date().toISOString().slice(0, 10),
        Validators.required,
      ],
  ...
  private createProfessionalCertificationGroup(
    value?: EmployeeProfessionalCertificationDto,
  ): FormGroup {
    return this.fb.group({
      id: [value?.id ?? null],
      certificationTitle: [
  ...
  private buildPayload(includeId: boolean): EmployeeDetailDto {
    ...
    const payload: any = {
      ...
      employeeFamilyInfos: raw.employeeFamilyInfos.filter(
        (item: any) => item.name || item.idGender || item.idRelationship,
      ),
      employeeEducationInfos: raw.employeeEducationInfos.filter(
        (item: any) => item.major || item.instituteName || item.idEducationLevel,
      ),
      employeeDocuments: raw.employeeDocuments.filter(
        (item: any) => item.documentName || item.uploadedFile,
      ),
      employeeProfessionalCertifications: raw.employeeProfessionalCertifications.filter(
        (item: any) => item.certificationTitle || item.certificationInstitute,
      ),
    };
